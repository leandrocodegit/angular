...
import { Parcela } from 'src/app/models/checkouts/Parcela';
...
declare var getParcelas:any

@Component({
  selector: 'spa-card2',
  templateUrl: './card2.component.html',
  styleUrls: ['./card2.component.scss', 'card2.component.css']
})
export class Card2Component implements OnInit, OnDestroy {
  ...
  validate(element: string): boolean {
    ...
    if (this.heroForm.get('nomeCartao')?.valid && element == 'nomeCartao')
      ...
      this.viewPortscroller.scrollToAnchor("form-parcelas"); 
    }
    ...
    if (this.heroForm.valid){
      this.formulario.token = gerarToken()
      this.isValid = true
      this.listaParcelas = getParcelas()

      this.listaParcelas.forEach(r => {
        console.log("Parcelas")
        console.log(r.parcelas)
        console.log(r.valorParcela)
        console.log(r.totalAmount)
      })
    }        
  ...
  setParcela(value: Parcela){
    this.formulario.installments = value.parcelas
    this.formulario.parcela = value
  }
